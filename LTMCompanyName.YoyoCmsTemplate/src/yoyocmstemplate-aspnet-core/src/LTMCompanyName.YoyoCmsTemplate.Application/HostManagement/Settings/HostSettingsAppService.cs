...
namespace LTMCompanyName.YoyoCmsTemplate.HostManagement.Settings
{
    ...
    public class HostSettingsAppService : SettingsAppServiceBase, IHostSettingsAppService
    {
        ...
        private async Task<TenantManagementSettingsEditDto> GetTenantManagementSettingsAsync()
        {
            ...
            {
                // 宿主注册租户
                AllowSelfRegistration = await SettingManager.GetSettingValueAsync<bool>(AppSettingNames.Host.AllowSelfRegistration),
                IsNewRegisteredTenantActiveByDefault = await SettingManager.GetSettingValueAsync<bool>(AppSettingNames.Host.IsNewRegisteredTenantActiveByDefault),
                // 宿主租户注册验证码
                UseCaptchaOnTenantRegistration = await SettingManager.GetSettingValueAsync<bool>(AppSettingNames.Host.UseCaptchaOnTenantRegistration),
                CaptchaOnTenantRegistrationType = (ValidateCodeType)await SettingManager.GetSettingValueAsync<int>(AppSettingNames.Host.CaptchaOnTenantRegistrationType),
                CaptchaOnTenantRegistrationLength = await SettingManager.GetSettingValueAsync<int>(AppSettingNames.Host.CaptchaOnTenantRegistrationLength),
            };

            var defaultEditionId = await SettingManager.GetSettingValueAsync(AppSettingNames.Host.DefaultEdition);
        ...
        private async Task<HostBillingSettingsEditDto> GetBillingSettingsAsync()
        {
            ...
            {
                LegalName = await SettingManager.GetSettingValueAsync(AppSettingNames.Host.BillingLegalName),
                Address = await SettingManager.GetSettingValueAsync(AppSettingNames.Host.BillingAddress)
            };
        }
        ...
        {
            await SettingManager.ChangeSettingForApplicationAsync(AppSettingNames.Host.BillingLegalName, input.LegalName);
            await SettingManager.ChangeSettingForApplicationAsync(AppSettingNames.Host.BillingAddress, input.Address);

...
namespace LTMCompanyName.YoyoCmsTemplate.MultiTenancy
{
    ...
    public class TenantManager : AbpTenantManager<Tenant, User>
    {
        ...
        public async Task<int> CreateWithAdminUser(
            string tenancyName,
            string name,
            ...
            string emailActivationLink)
        {
            ...
            using (var uow = _unitOfWorkManager.Begin(TransactionScopeOption.RequiresNew))
            {
                ...
                var tenant = new Tenant(tenancyName, name)
                {
                    IsActive = isActive,
                    EditionId = editionId,
                    SubscriptionEndUtc = subscriptionEndDate?.ToUniversalTime(),
                ...
                using (_unitOfWorkManager.Current.SetTenantId(tenant.Id))
                {
                    ...
                    adminUser.NeedToChangeThePassword = shouldChangePasswordOnNextLogin;
        ...
        public async Task CheckEditionAsync(int? editionId, bool isInTrialPeriod)
        {
            ...
            var error = LocalizationManager.GetSource(YoyoCmsTemplateConsts.LocalizationSourceName).GetString("FreeEditionsCannotHaveTrialVersions");
        ...
        public async Task<EndSubscriptionResult> EndSubscriptionAsync(Tenant tenant, SubscribableEdition edition, DateTime nowUtc)
        {
            ...
            var subscriptionEndDateUtc = tenant.SubscriptionEndUtc.Value;
            if (!tenant.IsInTrialPeriod)
            {
                subscriptionEndDateUtc = tenant.SubscriptionEndUtc.Value.AddDays(edition.WaitingDayAfterExpire ?? 0);
            }
            ...
            if (!tenant.IsInTrialPeriod && edition.ExpiringEditionId.HasValue)
            {
                ...
                tenant.SubscriptionEndUtc = null;

...
export class OrganizationUnitTreePanelComponent extends AppComponentBase
  ...
  treeDataMap(): NzTreeNode[] {
    ...
    const ouDtataParentIsNull = _.filter(
      this._ouData,
      item => (<OrganizationUnitListDto>item).parentId === null,
    );
  ...
  private _recursionGenerateTree(item: OrganizationUnitListDto): NzTreeNode {
    ...
    const childs = _.filter(
      this._ouData,
      child => (<OrganizationUnitListDto>child).parentId === item.id,
    );
    // 父节点 无返回undefined
    const parentOu = _.find(
      this._ouData,
      p => (<OrganizationUnitListDto>p).id === item.parentId,
    );
  ...
  unitCreated(ou: OrganizationUnitListDto): void {
    ...
    let childs = _.filter(
      this._ouData,
      child => (<OrganizationUnitListDto>child).parentId === ou.id,
    );
    const _treeNode = new NzTreeNode({
    ...
    if (this.activedNode) {
      ...
      childs = _.filter(
        this._ouData,
        child =>
          (<OrganizationUnitListDto>child).parentId ===
          // tslint:disable-next-line:radix

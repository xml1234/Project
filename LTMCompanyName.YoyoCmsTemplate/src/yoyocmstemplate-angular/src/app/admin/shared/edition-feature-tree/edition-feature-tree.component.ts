...
export class EditionFeatureTreeComponent extends AppComponentBase {
   ...
   */
  processsTreeData() {
    this.treeData = this.arrayService.arrToTreeNode(
      this._featureSourceData.features,
      {
        idMapName: 'name',
  ...
  fullTreeData(node: NzTreeNode, featureName: string) {
    let feature = this._featureSourceData.featureValues.find(item => item.name === featureName);
    // 默认值
    if (!feature) {
